...
INSERT INTO `clients` VALUES (1,'Mery Blanca','Chinchay Gomez','41663054','951376951','',NULL,'PAID','ACTIVE','2025-05-05 09:39:26.956751','2025-06-07 15:53:40.000000',NULL,NULL),(2,'Yuli Julissa','Rímac López','75524515','987780017','',NULL,'PAID','ACTIVE','2025-05-05 11:59:42.113875','2025-07-08 18:57:43.000000',NULL,NULL),(3,'Miguel Angel','Regaldo Palma','74357705','948954927','',NULL,'PAID','ACTIVE','2025-05-05 14:50:45.821721','2025-07-08 18:57:43.000000',NULL,NULL),(4,'Eberth Gabriel','Minaya Garcia','44825378','943900818','',NULL,'PAID','ACTIVE','2025-05-05 20:20:40.249991','2025-06-11 23:09:09.000000',NULL,NULL),(5,'Grescia ','Evangelina Salvador','60858237','991438074','',NULL,'PAID','ACTIVE','2025-05-05 20:26:42.541257','2025-06-11 23:09:09.000000',NULL,NULL),(6,'Edinson ','Atoc Obregon','71926248','928028535','',NULL,'PAID','ACTIVE','2025-05-05 20:30:56.269864','2025-06-11 23:09:09.000000',NULL,NULL),(7,'Lenin','Linares Leiva','75671429','938231126','',NULL,'PAID','ACTIVE','2025-05-08 22:47:28.559995','2025-06-11 23:09:09.000000',NULL,NULL),(8,'Hiber Victor ','Alva Pachas','33342491','943499360','',NULL,'PAID','ACTIVE','2025-05-09 14:17:06.679041','2025-05-09 14:17:06.679041',NULL,NULL),(9,'Flor ','Maguiña Ayala','45889852','910670128','',NULL,'PAID','ACTIVE','2025-05-09 14:26:40.545883','2025-05-23 17:36:49.000000',NULL,NULL),(10,'Keyvin Justo','Minaya Picon','75261732','935224372','',NULL,'PAID','ACTIVE','2025-05-09 14:32:25.642064','2025-07-08 18:57:43.000000',NULL,NULL),(11,'Alex Manuel ','Blas Colonia ','46262455','976732200','',NULL,'PAID','ACTIVE','2025-05-09 14:44:18.073177','2025-07-08 18:57:43.000000',NULL,NULL),(12,'Daniel Jesus ','Chavez Javier','44049492','978208097','',NULL,'PAID','ACTIVE','2025-05-09 14:55:28.852989','2025-06-11 23:09:09.000000',NULL,NULL),(13,'Denis Patricia ','Cashpa Vega','47104542','924132661','',NULL,'PAID','ACTIVE','2025-05-09 18:58:36.544478','2025-07-08 18:54:08.000000',NULL,NULL);
...
--
-- Table structure for table `migration_dependencies`
--
...
CREATE TABLE `migration_dependencies` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `migration_id` int(11) NOT NULL COMMENT 'ID de la migraci├│n que depende',
  `depends_on_migration_id` int(11) NOT NULL COMMENT 'ID de la migraci├│n de la que depende',
  `dependency_type` enum('required','optional','conflicts') NOT NULL DEFAULT 'required' COMMENT 'Tipo de dependencia',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT 'Fecha de creaci├│n',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_migration_dependency` (`migration_id`,`depends_on_migration_id`),
  KEY `idx_migration_dependencies_depends_on` (`depends_on_migration_id`),
  CONSTRAINT `fk_migration_dependencies_depends_on` FOREIGN KEY (`depends_on_migration_id`) REFERENCES `migrations` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_migration_dependencies_migration` FOREIGN KEY (`migration_id`) REFERENCES `migrations` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Dependencias entre migraciones';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `migration_dependencies`
--

LOCK TABLES `migration_dependencies` WRITE;
/*!40000 ALTER TABLE `migration_dependencies` DISABLE KEYS */;
INSERT INTO `migration_dependencies` VALUES (1,2,1,'required','2025-07-09 01:52:15');
/*!40000 ALTER TABLE `migration_dependencies` ENABLE KEYS */;
UNLOCK TABLES;
...
CREATE TABLE `migration_logs` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `migration_id` int(11) NOT NULL COMMENT 'ID de la migraci├│n relacionada',
  `log_level` enum('info','warning','error','success') NOT NULL DEFAULT 'info' COMMENT 'Nivel del log',
  `message` text NOT NULL COMMENT 'Mensaje del log',
  `details` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL COMMENT 'Detalles adicionales en formato JSON' CHECK (json_valid(`details`)),
  `timestamp` timestamp NOT NULL DEFAULT current_timestamp() COMMENT 'Timestamp del log',
  `execution_step` varchar(100) DEFAULT NULL COMMENT 'Paso de ejecuci├│n donde ocurri├│ el log',
  PRIMARY KEY (`id`),
  KEY `idx_migration_logs_migration_id` (`migration_id`),
  KEY `idx_migration_logs_timestamp` (`timestamp`),
  KEY `idx_migration_logs_level` (`log_level`),
  CONSTRAINT `fk_migration_logs_migration` FOREIGN KEY (`migration_id`) REFERENCES `migrations` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=17 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Logs detallados de las migraciones';
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `migration_logs` VALUES (1,1,'success','Sistema de migraciones creado exitosamente',NULL,'2025-07-09 01:50:10','finalization'),(2,2,'info','Inicio de migraci├│n de estructura de base de datos',NULL,'2025-07-09 01:52:15','start'),(3,2,'info','Verificando tablas existentes antes de la migraci├│n',NULL,'2025-07-09 01:52:15','verification'),(4,2,'info','Creaci├│n de tabla clients completada',NULL,'2025-07-09 01:52:15','table_creation'),(5,2,'info','Creaci├│n de tabla installations completada',NULL,'2025-07-09 01:52:15','table_creation'),(6,2,'info','Creaci├│n de tabla client_payment_configs completada',NULL,'2025-07-09 01:52:15','table_creation'),(7,2,'info','Creaci├│n de tabla devices completada',NULL,'2025-07-09 01:52:15','table_creation'),(8,2,'info','Creaci├│n de tabla employees completada',NULL,'2025-07-09 01:52:15','table_creation'),(9,2,'info','Creaci├│n de tabla payments completada',NULL,'2025-07-09 01:52:15','table_creation'),(10,2,'info','Creaci├│n de tabla payment_histories completada',NULL,'2025-07-09 01:52:15','table_creation'),(11,2,'info','Configuraci├│n de restricciones de clave for├ínea completada',NULL,'2025-07-09 01:52:15','constraints'),(12,2,'info','Creaci├│n de ├¡ndices para optimizaci├│n completada',NULL,'2025-07-09 01:52:15','indexes'),(13,2,'info','Migraci├│n de datos de client a clients completada',NULL,'2025-07-09 01:52:15','data_migration'),(14,2,'info','Migraci├│n de datos de payment a payments completada',NULL,'2025-07-09 01:52:15','data_migration'),(15,2,'info','Migraci├│n de datos de payment_history a payment_histories completada',NULL,'2025-07-09 01:52:15','data_migration'),(16,2,'success','Migraci├│n de estructura completada exitosamente',NULL,'2025-07-09 01:52:15','finalization');
...
CREATE TABLE `migrations` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `version` varchar(255) NOT NULL COMMENT 'Versi├│n de la migraci├│n (ej: 001, 002)',
  `name` varchar(255) NOT NULL COMMENT 'Nombre descriptivo de la migraci├│n',
  `description` text DEFAULT NULL COMMENT 'Descripci├│n detallada de los cambios',
  `sql_file` varchar(255) NOT NULL COMMENT 'Archivo SQL ejecutado',
  `execution_time` decimal(10,3) DEFAULT NULL COMMENT 'Tiempo de ejecuci├│n en segundos',
  `status` enum('pending','running','completed','failed','rolled_back') NOT NULL DEFAULT 'pending' COMMENT 'Estado de la migraci├│n',
  `error_message` text DEFAULT NULL COMMENT 'Mensaje de error si fall├│',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT 'Fecha de creaci├│n del registro',
  `executed_at` timestamp NULL DEFAULT NULL COMMENT 'Fecha de ejecuci├│n',
  `rolled_back_at` timestamp NULL DEFAULT NULL COMMENT 'Fecha de rollback',
  `executed_by` varchar(100) DEFAULT NULL COMMENT 'Usuario que ejecut├│ la migraci├│n',
  `checksum` varchar(64) DEFAULT NULL COMMENT 'Checksum del archivo SQL para verificar integridad',
  `backup_file` varchar(255) DEFAULT NULL COMMENT 'Archivo de backup creado antes de la migraci├│n',
  `affected_tables` text DEFAULT NULL COMMENT 'Tablas afectadas por la migraci├│n (JSON)',
  `affected_rows` int(11) DEFAULT NULL COMMENT 'N├║mero de filas afectadas',
  PRIMARY KEY (`id`),
...
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Registro de migraciones de base de datos';
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `migrations` VALUES (1,'001','Crear sistema de registro de migraciones','Creaci├│n inicial del sistema de control de versiones de base de datos con tablas migrations, migration_logs y migration_dependencies','create-migrations-table.sql',NULL,'completed',NULL,'2025-07-09 01:50:10','2025-07-09 01:50:10',NULL,'system',NULL,NULL,'[\"migrations\", \"migration_logs\", \"migration_dependencies\"]',NULL),(2,'002','Migraci├│n de estructura de base de datos a TypeORM','Migraci├│n completa de la estructura de base de datos de tablas simples a estructura normalizada compatible con TypeORM. Incluye creaci├│n de nuevas tablas: clients, installations, client_payment_configs, devices, employees, payments, payment_histories con relaciones y restricciones de integridad referencial.','migrate-database-structure-fixed.sql',NULL,'completed',NULL,'2025-07-09 01:52:15','2025-07-09 01:52:15',NULL,'system',NULL,NULL,'[\"clients\", \"installations\", \"client_payment_configs\", \"devices\", \"employees\", \"payments\", \"payment_histories\"]',13);
...
-- Dump completed on 2025-07-08 21:37:44
